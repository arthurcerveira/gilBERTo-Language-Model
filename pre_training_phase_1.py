...
CHECKPOINT = "./gilBERTo-phase-2-model"
...
# Create single training file if not exists
if not os.path.isfile("oscar.txt"):
    folder_path = './oscar-data'
    file_paths = glob.glob(os.path.join(folder_path, "*.txt"))

    with open("oscar.txt", "wb") as outfile:
...
class IterableLineByLineTextDataset(IterableDataset):
    ...
    def __len__(self):
        if self.file_path == "./Validation.txt":
            return 7_851_663
        elif self.file_path=="./oscar.txt":
            return 64_640_252  # 59_615_919 brWaC

        return None

train_data = IterableLineByLineTextDataset(
    file_path="./oscar.txt", tokenizer=tokenizer, block_size=128
)

validation = IterableLineByLineTextDataset(
    tokenizer=tokenizer, file_path="./Validation.txt", block_size=128
)
...
trainer.save_model("./gilBERTo-phase-3-model")
