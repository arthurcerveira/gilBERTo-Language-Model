...
MODELS = ["BERTimbau", "BertPT", "M-BERT", "gilBERTo-phase-1", "gilBERTo-phase-2", "gilBERTo-phase-3"]
DATASETS = ["total", "selective"]
CRF = [False, True]


def compare_ner(models, dataset="total", use_crf=False):
    ...
    output_file = f"ner_bert_{dataset}_{epochs}.csv"
    ...
    for model in models:
        ...
        print(
            f"[{datetime.now():%H:%M}] Running {model} on {dataset} dataset with CRF={use_crf}"
        )
...
if __name__ == "__main__":
    for dataset, use_crf in product(DATASETS, CRF):
        compare_ner(MODELS, dataset, use_crf)
